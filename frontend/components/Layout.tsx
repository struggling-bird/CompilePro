...
import { Settings, LogOut, Layers, HardDrive, Globe, Users, LayoutTemplate, UserCog, Menu, X, ChevronLeft, ChevronRight, ShieldCheck, ChevronDown, User } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange, userEmail, onLogout }) => {
  ...
  const navConfig: NavItemConfig[] = [
    ...
    { id: 'templates', labelKey: 'templates', icon: LayoutTemplate, tab: TabView.TEMPLATES },
    { id: 'manage', labelKey: 'manage', icon: HardDrive, tab: TabView.MANAGE },
    { id: 'customers', labelKey: 'customers', icon: Users, tab: TabView.CUSTOMERS },
    { 
      id: 'team', 
      labelKey: 'team', 
      icon: UserCog, 
      children: [
        { id: 'members', labelKey: 'members', icon: User, tab: TabView.MEMBERS },
        { id: 'roles', labelKey: 'roles', icon: ShieldCheck, tab: TabView.ROLES }
      ]
    }
  ];
  ...
  useEffect(() => {
    navConfig.forEach(item => {
      if (item.children && isParentActive(item) && !expandedMenus.includes(item.id)) {
  ...
  const renderNavItem = (item: NavItemConfig, level = 0) => {
        ...
        <div key={item.id} className="mb-1">
          <button
            ...
            className={`
              ...
              ${isCollapsed ? 'justify-center py-3 px-0 mx-2' : 'px-3 py-2.5 mx-2'}
              ${
            ...
            `}
          >
            <Icon className={`flex-shrink-0 transition-all duration-300 ${isCollapsed ? 'w-6 h-6' : 'w-5 h-5 mr-3'} ${hasActiveChild ? 'text-blue-400' : ''}`} />
            
            <span className={`font-medium text-sm tracking-wide whitespace-nowrap overflow-hidden transition-all duration-300 flex-1 text-left ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>
              {label}
            </span>

            {!isCollapsed && (
              <ChevronDown className={`w-4 h-4 text-slate-500 transition-transform duration-300 ${isExpanded ? 'transform rotate-180' : ''}`} />
            )}
    ...
    return (
      <button
        ...
        `}
      >
        <Icon className={`flex-shrink-0 transition-all duration-300 ${isCollapsed ? 'w-6 h-6' : 'w-4.5 h-4.5 mr-3'} ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-300'}`} />
        
        <span className={`font-medium text-sm tracking-wide whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>
          {label}
        </span>
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden">
      {/* Mobile Overlay */}
      <div 
        className={`fixed inset-0 bg-black/50 z-20 md:hidden transition-opacity duration-300 ${isSidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      ...
      `}>
        {/* Sidebar Header */}
        <div className={`flex items-center h-16 border-b border-slate-800/50 transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'px-6'}`}>
          ...
          <span className={`font-bold text-xl tracking-tight ml-3 text-slate-100 whitespace-nowrap transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'w-auto opacity-100'}`}>
            ZhugeIO
          </span>
        ...
        <div className="p-3 border-t border-slate-800/50 bg-[#0B1120]">
           <div className="space-y-1 mb-4">
             <button
                ...
                `}
              >
                <Settings className={`${isCollapsed ? 'w-5 h-5' : 'w-4.5 h-4.5 mr-3'} transition-all`} />
                <span className={`text-sm font-medium transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>
                  {t.layout.settings}
                </span>
                {isCollapsed && (
                  <div className="absolute left-full top-1/2 -translate-y-1/2 ml-3 px-2.5 py-1.5 bg-slate-800 text-white text-xs font-medium rounded-md opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 shadow-xl border border-slate-700 transition-opacity">
              ...
              <button
                ...
                `}
              >
                <LogOut className={`${isCollapsed ? 'w-5 h-5' : 'w-4.5 h-4.5 mr-3'} transition-all`} />
                <span className={`text-sm font-medium transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100'}`}>
                  {t.layout.logout}
                </span>
                {isCollapsed && (
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-50 transition-all duration-300">
        ...
        <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 h-16 flex items-center justify-between px-6 shrink-0 shadow-sm z-10 sticky top-0">
           <div className="flex items-center">
              <button className="md:hidden mr-4 p-2 -ml-2 text-slate-500 hover:bg-slate-100 rounded-md" onClick={() => setIsSidebarOpen(true)}>
              ...
              <h2 className="text-xl font-bold text-slate-800 hidden sm:block animate-in fade-in slide-in-from-left-2 tracking-tight">
                ...
                {(activeTab === TabView.MEMBERS || activeTab === TabView.ROLES) && t.layout.team}
           ...
           <div className="flex items-center space-x-3 sm:space-x-4">
              ...
              <div className="flex items-center space-x-3 pl-1">
                 ...
                 <div className="hidden sm:flex flex-col items-start">
                    <span className="text-sm font-semibold text-slate-700 leading-none">{userEmail.split('@')[0]}</span>
