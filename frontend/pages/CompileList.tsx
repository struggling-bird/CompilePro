          ...
          <div className="bg-white rounded-lg shadow-xl w-96 p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-slate-800">
                {t.compileList.createProjectTitle}
              </h3>
              <button onClick={() => setIsModalOpen(false)}>
                <i className="w-5 h-5 text-slate-400 hover:text-slate-600" />
              </button>
            </div>
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-100">
            <tr>
              <th
                scope="col"
                className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider w-16"
              >
      ...
      <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center gap-4">
        ...
        >
          {selectedIds.length === projects.length && projects.length > 0
            ? t.compileList.deselectAll
            : t.compileList.selectAll}
        </button>
