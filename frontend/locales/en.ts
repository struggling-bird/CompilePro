export default {
  common: {
    saved: "Saved successfully",
    success: "Success",
  },
  login: {
    title: "Login to ZhugeIO",
    emailPlaceholder: "Email address",
    passwordPlaceholder: "Password",
    loginBtn: "Login",
    autoLogin: "Auto login next time",
    forgotPassword: "Forgot password?",
    noAccount: "Don't have an account?",
    register: "Register now",
  },
  register: {
    title: "Create an Account",
    emailPlaceholder: "Email address",
    usernamePlaceholder: "Username",
    passwordPlaceholder: "Password",
    confirmPasswordPlaceholder: "Confirm Password",
    registerBtn: "Register",
    hasAccount: "Already have an account?",
    login: "Login now",
  },
  layout: {
    metaProjects: "Meta Projects",
    templates: "Templates",
    compilations: "Compilations",
    customers: "Customers",
    builds: "Builds",
    team: "Team",
    members: "Members",
    roles: "Roles",
    settings: "Account Settings",
    systemSettings: "System Settings",
    logout: "Logout",
    footer: "© 2023 ZhugeIO. All rights reserved.",
  },
  projectList: {
    searchPlaceholder: "Search projects...",
    newProject: "New Project",
    settings: "Settings",
    select: "SELECT",
    projectName: "PROJECT NAME",
    latestVersion: "LATEST VERSION",
    docs: "DOCS",
    buildDoc: "BUILD DOC",
    action: "ACTION",
    edit: "Edit",
    deleteConfirm:
      "Are you sure to delete this project and its workspace files?",
    deselectAll: "Deselect All",
    selectAll: "Select All",
    applySelection: "Apply Selection",
    createProjectTitle: "Create New Project",
    initialVersion: "Initial Version",
    createBtn: "Create Project",
    projectPlaceholder: "e.g. web-analytics",
    versionPlaceholder: "e.g. 1.0.0",
  },
  projectDetail: {
    back: "Back",
    gitRepo: "Git Repo",
    name: "NAME",
    type: "TYPE",
    fileLocation: "FILE LOCATION",
    description: "DESCRIPTION",
    action: "ACTION",
    edit: "Edit",
    delete: "Delete",
    addConfig: "Add Configuration Item",
    compilationCommands: "Compilation Commands",
    newCmd: "New Cmd",
    configTypesTitle: "Configuration Types:",
    configTypes: {
      text: "Text: Text content replacement (supports RegEx).",
      file: "File: File replacement (e.g. logo).",
      json: "JSON: JSON content modification (merge strategy).",
    },
    versionHistory: "Version History",
    deprecated: "Deprecated",
    new: "New",
    readme: "README",
    build: "BUILD",
    update: "UPDATE",
    docsTitle: "Project Documentation",
    docsDesc1:
      "The timeline above allows switching between different versions.",
    docsDesc2:
      "Documentation below will update automatically based on the selected version.",
    addVersionTitle: "Add Version / Branch",
    versionNo: "Version Name",
    source: "Source Version",
    versionType: "Type",
    tag: "Release (Tag)",
    branch: "Branch",
    save: "Save",
    cancel: "Cancel",
    textReplace: "Text Replace",
    fileReplace: "File Replace",
    jsonReplace: "JSON Replace",
    regexLabel: "REGULAR EXPRESSION",
    globalLabel: "Global",
    descLabel: "DESCRIPTION",
    replacementLabel: "REPLACEMENT VALUE",
    previewComment: "// Preview of:",
    uploadText: "Click or drag file to upload replacement",
    supports: "Supports: PNG, JPG, SVG",
    currentAsset: "Current Asset Preview",
    defaultConfig: "Default Configuration",
    customOverlay: "Custom Overlay",
    editable: "Editable",
    configDesc: {
      domain: "Platform main domain",
      logo: "Logo replacement",
      package: "Dependency updates",
    },
    createdAt: "CREATED AT",
    loadFileListFailed: "Failed to load file list",
    loadFileContentFailed: "Failed to read file content",
    fileList: "File List",
    noFiles: "No files found or not loaded",
    configName: "Config Name",
    inputConfigName: "Please input config name",
    configDescTitle: "Config Description",
    inputRegex: "Please input regex",
    matchIndex: "Match Index (starts from 0)",
    mappingType: "Mapping Type",
    targetValue: "Target Value",
    selectGlobalConfig: "Select Global Config",
    selectGlobalConfigPlaceholder: "Please select global config",
    selectGlobalTextConfig: "Select a global text config",
    previewContent: "Preview Content",
    matchCount: "Matched {{count}} items",
    selectFile: "Please select file",
    manualUpload: "Manual Upload",
    selectGlobalFileConfig: "Select a global file config",
    targetFile: "Target File",
    targetGroup: "Target Group",
    targetGroupTooltip: "Select which capture group to replace. 0 means the whole match.",
    group0: "Group 0 (Whole Match)",
    groupN: "Group {{n}}",
  },

  settings: {
    title: "Settings",
    accountSettings: "Account Settings",
    install: "Install",
    username: "Username",
    password: "Password",
    confirmPassword: "Confirm Password",
    email: "Email",
    gitBinding: "Git Binding",
    gitName: "Git Name",
    apiEndpoint: "API Endpoint",
    accessToken: "Access Token",
    howToGetToken: "How to get token?",
    sshKey: "SSH Key",
    copyKey: "Copy Key",
    pushSettings: "Push Settings",
    enableEmail: "Enable Email Notifications",
    enableSms: "Enable SMS Notifications",
    enableWechat: "Enable WeChat Notifications",
    saveAll: "Save All Changes",
    systemCheck: "System Check",
    checkEnvironment: "Check Environment",
    recheck: "Recheck",
    checking: "Checking...",
    installed: "Installed",
    notInstalled: "Not Installed",
    version: "Version",
    versionManager: "Version Manager",
    managerInstalled: "Installed",
    managerNotInstalled: "Not Installed",
    systemConfig: "File Storage Settings",
    saveChanges: "Save Changes",
    refresh: "Refresh",
    noConfigs: "No configurations found",
    configTitle: "System Configuration",
    saveSuccess: "Configuration saved successfully",
    saveFailed: "Failed to save configuration",
    loadFailed: "Failed to load configs",
    workspaceStats: "Workspace Stats",
    totalUsers: "Total Users",
    totalProjects: "Total Projects",
    rootPath: "Root Path",
    userCount: "User Count",
    totalSize: "Total Size",
    storageDistribution: "Storage Distribution",
    projectCountDistribution: "Project Count Distribution",
    details: "Details",
    usernameColumn: "Username",
    projectCountColumn: "Project Count",
    sizeColumn: "Size",
    projectName: "Project Name",
    projectDesc: "Description",
    // Storage Management
    storageAnalysis: "Storage Analysis",
    fileExplorer: "File Explorer",
    spaceManagement: "Space Management",
    configuration: "Configuration",

    // Storage Overview
    storageGrowthTrend: "Storage Growth Trend",
    fileTypeDistribution: "File Type Distribution",
    hotFilesAccess: "Hot Files Access",
    accessCount: "Access Count",

    // Quota Management
    storageQuotaUsage: "Storage Quota Usage",
    totalSpace: "Total Space",
    usedSpace: "Used Space",
    remaining: "Remaining",
    usageByDepartment: "Usage by Department",
    usageByProject: "Usage by Project",
    cleanupSuggestions: "Cleanup Suggestions",
    largeFiles: "Large Files (>100MB)",
    tempFiles: "Temp Files",
    logFiles: "Log Files",
    clean: "Clean",
    department: "Department",
    used: "Used",
    quota: "Quota",
    usage: "Usage",
    project: "Project",

    // File Explorer
    searchFiles: "Search files...",
    upload: "Upload",
    newFolder: "New Folder",
    fileName: "Name",
    fileSize: "Size",
    fileType: "Type",
    modifiedTime: "Modified",
    action: "Action",
    preview: "Preview",
    download: "Download",
    delete: "Delete",
    filePreview: "File Preview",
    filePath: "Path",
    lastAccessed: "Last Accessed",
    warningThreshold: "Warning Threshold (%)",
    potentialSaving: "Potential saving:",
    cache: "Cache",
    tempFilesDesc: "System temporary files older than 7 days",
    logFilesDesc: "Archived application logs older than 30 days",
    cacheDesc: "Browser and application cache",
    previewNotAvailable: "Preview not available for this file type",
    close: "Close",
    deleteConfirm: "Are you sure you want to delete this file?",
    yes: "Yes",
    no: "No",
  },
  templateList: {
    title: "Project Templates",
    author: "Author",
    createdDate: "Created Date",
    newTemplate: "New Template",
    name: "TEMPLATE NAME",
    latestVersion: "LATEST VERSION",
    description: "DESCRIPTION",
    date: "CREATED DATE",
    action: "ACTION",
    edit: "Edit",
    delete: "Delete",
    searchPlaceholder: "Search templates...",
    deleteConfirm: "Are you sure to delete this template?",
    deleteSuccess: "Template deleted",
    deleteFailed: "Delete failed",
    updateTime: "Update Time",
    updater: "Updater",
    expand: "Expand",
    collapse: "Collapse",
    search: "Search",
    reset: "Reset",
    totalItems: "Total {{count}} items",
    fetchFailed: "Failed to fetch templates",
    toggleNotImpl: "Toggle status not implemented yet",
    namePlaceholder: "Template Name",
    creatorPlaceholder: "Creator",
    descPlaceholder: "Description",
  },
  templateDetail: {
    back: "Back",
    newTitle: "Create Template Suite",
    editTitle: "Edit Template Suite",
    loadFailed: "Failed to load template",
    newTemplate: "New Template",
    fetchVersionFailed: "Failed to fetch version details",
    merged: "Merged {{source}} into {{target}}",
    baseNotFound: "Base version not found",
    noBase: "No base version to merge into",
    createSuccess: "Template created successfully",
    createFailed: "Failed to create template",
    newVersionSuccess: "New version created successfully",
    deleteLastVersion: "Cannot delete the only version",
    deleteVersionTitle: "Delete Version",
    deleteVersionConfirm: "Are you sure you want to delete this version? This action cannot be undone.",
    versionDeleted: "Version deleted",
    confirmDelete: "Confirm Delete",
    deleteGlobalConfirm: "Are you sure you want to delete this global config?",
    globalDeleted: "Global config deleted",
    deleteGlobalFailed: "Failed to delete global config",
    moduleSaved: "Module config saved",
    saveModuleFailed: "Failed to save module config",
    globalAdded: "Global config added successfully",
    saveGlobalFailed: "Failed to save global config",
    addModuleFailed: "Failed to add module",
    versionUpdated: "Version updated",
    switchVersionFailed: "Failed to switch version",
    versionGraph: "Version Graph",
    versionStatus: "Version {{status}}",
    enabled: "enabled",
    disabled: "disabled",
    deprecatedReason: "Deprecated Reason",
    updateSuccess: "Template updated",
    updateFailed: "Update failed",
    globalConfigTitle: "Global Configuration",
    addGlobalConfig: "Add Config Item",
    name: "NAME",
    defaultValue: "DEFAULT VALUE",
    desc: "DESCRIPTION",
    isHidden: "HIDDEN",
    action: "ACTION",
    yes: "Yes",
    no: "No",
    edit: "Edit",
    delete: "Delete",
    moduleConfigTitle: "Module Configuration",
    select: "SELECT",
    fileLocation: "FILE LOCATION",
    fileLocationPlaceholder: "e.g., /src/config.js",
    regex: "REGEX",
    mapping: "DEFAULT VALUE",
    publishMethod: "Publish Method",
    gitPush: "Publish to Git",
    download: "Download",
    addModule: "Add Module",
    addBranch: "Add Branch Version",
    timelineNote: "Click markers to select version source",
    branchNoteTitle: "Branching Rules",
    branchNote:
      "After adding a sub-branch, it can be modified. Rebase to main branch is supported.",
    addModuleTitle: "Add Module Project",
    selectProject: "Select Project",
    selectVersion: "Select Version",
    add: "Add",
    save: "Save",
    cancel: "Cancel",
    mapToGlobal: "Global Config",
    fixedValue: "Fixed Value",
    manualInput: "Manual Input",
    noData: "No Data",
    usedBy: "Used By",
    docsTitle: "Project Documentation",
    build: "Build Doc",
    update: "Update Doc",
    newVersion: "New Version",
    parentVersion: "Parent Version",
    versionType: "Version Type",
    versionNumber: "Version Number",
    versionDesc: "Description",
    inputBranchName: "Please input branch name",
    branchNamePattern: "Only alphanumeric and underscore allowed",
    branchNamePlaceholder: "branch_name",
    switchVersionTitle: "Switch Module Version",
    createdAt: "CREATED AT",
    types: {
      Major: "Major",
      Minor: "Minor",
      Patch: "Patch",
      Hotfix: "Hotfix",
      Branch: "Branch",
    },
    editGlobalConfig: "Edit Global Config",
    createGlobalConfig: "Create Global Config",
    editModuleConfig: "Edit Module Config",
    createModuleConfig: "Create Module Config",
    inputName: "Please input name",
    uploadFile: "Please upload a file",
    clickToUpload: "Click to Upload",
    inputDefaultValue: "Please input default value",
    mergeToParent: "Merge to Parent",
    enableVersion: "Enable Version",
    disableVersion: "Disable Version",
    deleteVersionDependencyWarning: "Cannot delete version with downstream dependencies",
    zoomIn: "Zoom In (Ctrl + +)",
    zoomOut: "Zoom Out (Ctrl + -)",
    fitView: "Fit View",
    actualSize: "Actual Size (100%)",
    handTool: "Hand Tool (Drag Canvas)",
    disableVersionTitle: "Disable Version",
    reason: "Reason",
    inputReason: "Please input reason",
    reasonLength: "Reason must be at least 10 characters",
    reasonPlaceholder: "Please explain why this version is being disabled...",
     confirm: "Confirm",
     versionNotFound: "Version not found",
   },
  compilationList: {
    title: "Compilations",
    newCompilation: "New Compilation",
    searchPlaceholder: "Search compilations...",
    name: "NAME",
    template: "TEMPLATE",
    templateVersion: "TEMPLATE VERSION",
    customer: "CUSTOMER",
    environment: "ENVIRONMENT",
    status: "STATUS",
    lastBuildTime: "LAST BUILD TIME",
    lastBuilder: "EXECUTOR",
    createdAt: "CREATED AT",
    createdBy: "CREATED BY",
    action: "ACTION",
    edit: "Edit",
    delete: "Delete",
    history: "History",
    execute: "Execute",
    deleteConfirm: "Are you sure to delete this compilation?",
    createSuccess: "Created Successfully",
    createFailed: "Create failed",
    fetchFailed: "Failed to fetch compilations",
    executeFeature: "Execute feature coming soon",
    historyFeature: "History feature coming soon",
    deleteFailed: "Delete failed",
    search: "Search",
    reset: "Reset",
  },
  compilationDetail: {
    newTitle: "New Compilation Config",
    editTitle: "Edit Compilation Config",
    back: "Back",
    basicInfo: "Basic Information",
    name: "Compilation Name",
    template: "Select Template",
    templateVersion: "Select Version",
    customer: "Select Customer",
    environment: "Select Environment",
    globalConfigTitle: "Global Configuration",
    moduleConfigTitle: "Module Configuration",
    save: "Save",
    cancel: "Cancel",
    saveSuccess: "Saved Successfully",
    module: "Module",
    configName: "Config Name",
    defaultValue: "Default Value",
    currentValue: "Current Value",
    targetValue: "Target Value",
    manualInput: "Manual Input",
    override: "Override",
    useDefault: "Use Default",
    detail: "Detail",
    desc: "Description",
    compileType: "Compile Type",
    privateDeploy: "Private Deployment",
    publicCloud: "Public Cloud",
    publishMethod: "Publish Method",
    gitPush: "Git Push",
    download: "Download",
    auto: "Auto",
    sharedMembers: "Shared Members",
    notification: "Notification Settings",
    push: "Push",
    sms: "SMS",
    wechat: "WeChat",
    other: "Other",
    selectCustomer: "Select Customer",
    selectEnvironment: "Select Environment",
    addConfig: "Add Config Item",
    loadDataFailed: "Failed to load data",
    loading: "Loading...",
    loadTemplateFailed: "Failed to load template details",
    saveFailed: "Save failed",
    basicInfoUpdated: "Basic info updated",
    loadFormFailed: "Failed to load form data",
    loadVersionsFailed: "Failed to load versions",
    loadEnvsFailed: "Failed to load environments",
    enterName: "Please enter name",
    namePlaceholder: "e.g. Online Deployment V1",
    selectTemplatePlaceholder: "Select Template",
    selectVersionPlaceholder: "Version",
    selectEnvPlaceholder: "Environment",
  },
  builds: {
    title: "New Build",
    configTitle: "Build Configuration",
    building: "Building in progress...",
    customer: "Customer",
    selectCustomer: "Select Customer",
    environment: "Environment",
    selectEnvironment: "Select Environment",
    compilation: "Compilation",
    selectCompilation: "Select Compilation",
    desc: "Build Description",
    descPlaceholder: "Detailed description (min 20 chars)...",
    descRequired: "Description is required",
    descMin: "Min 20 chars",
    startBuild: "Start Build",
    targetModules: "Target Modules",
    available: "Available",
    selectAll: "Select All",
    clear: "Clear",
    selectCompFirst: "Please select a compilation first",
    noDesc: "No description",
    downloadArtifact: "Download Artifact",
    moduleStatus: "Module Status",
    downloadModuleArtifact: "Download Module Artifact",
    buildLogs: "Build Logs",
    noLogs: "No logs available",
    loadCustomersFailed: "Failed to load customers",
    loadEnvsFailed: "Failed to load environments",
    loadCompsFailed: "Failed to load compilations",
    loadCompDetailFailed: "Failed to load compilation details",
    selectModuleRequired: "Please select at least one module",
    startBuildFailed: "Failed to start build",
  },
  customerList: {
    searchPlaceholder: "Search customers...",
    newCustomer: "New Customer",
    name: "NAME",
    contact: "CONTACT PERSON",
    phone: "PHONE",
    email: "EMAIL",
    status: "STATUS",
    contractDate: "CONTRACT DATE",
    action: "ACTION",
    edit: "Edit",
    delete: "Delete",
    environments: "Environments",
  },
  customerDetail: {
    back: "Back",
    newTitle: "New Customer",
    editTitle: "Edit Customer",
    basicInfo: "Basic Information",
    name: "Customer Name",
    contact: "Contact Person",
    phone: "Phone Number",
    email: "Email Address",
    address: "Address",
    contractDate: "Contract Date",
    status: "Status",
    statusActive: "Active",
    statusInactive: "Inactive",
    deployments: "Associated Deployments",
    save: "Save Customer",
    cancel: "Cancel",
  },
  memberList: {
    searchPlaceholder: "Search team members...",
    newMember: "New Member",
    name: "NAME",
    email: "EMAIL",
    role: "ROLE",
    status: "STATUS",
    joinDate: "JOIN DATE",
    action: "ACTION",
    edit: "Edit",
    delete: "Delete",
  },
  memberDetail: {
    back: "Back",
    newTitle: "New Team Member",
    editTitle: "Edit Team Member",
    basicInfo: "Member Information",
    name: "Full Name",
    email: "Email Address",
    role: "Role",
    roles: {
      admin: "Admin",
      developer: "Developer",
      viewer: "Viewer",
    },
    status: "Status",
    statusActive: "Active",
    statusInactive: "Inactive",
    joinDate: "Join Date",
    save: "Save Member",
    cancel: "Cancel",
  },
  roleList: {
    searchPlaceholder: "Search roles...",
    newRole: "New Role",
    name: "ROLE NAME",
    description: "DESCRIPTION",
    permissions: "PERMISSIONS",
    action: "ACTION",
    edit: "Edit",
    delete: "Delete",
    systemRole: "System Role",
  },
  roleDetail: {
    back: "Back",
    newTitle: "New Role",
    editTitle: "Edit Role",
    basicInfo: "Basic Information",
    name: "Role Name",
    description: "Description",
    permissions: "Permissions Configuration",
    save: "Save Role",
    cancel: "Cancel",
    selectAll: "Select All",
    deselectAll: "Deselect All",
  },
  environment: {
    title: "Environments",
    listTitle: "Environment List",
    add: "Add Environment",
    edit: "Edit Environment",
    detail: "Environment Detail",
    deleteConfirm: "Are you sure you want to delete this environment?",
    deleteSuccess: "Environment deleted successfully",
    saveSuccess: "Environment saved successfully",
    basicInfo: "Basic Information",
    name: "Environment Name",
    url: "URL",
    account: "Access Account",
    password: "Access Password",
    supportRemote: "Support Remote",
    remoteMethod: "Remote Method",
    remark: "Remark",
    nodes: "Nodes",
    addNode: "Add Node",
    editNode: "Edit Node",
    deleteNodeConfirm: "Are you sure you want to delete this node?",
    deleteNodeSuccess: "Node deleted successfully",
    saveNodeSuccess: "Node saved successfully",
    ip: "IP",
    host: "Host Name",
    domain: "Domain",
    os: "OS",
    chip: "Chip",
    cpu: "CPU",
    memory: "Memory",
    diskType: "Disk Type",
    diskSize: "Disk Size",
    credentials: "Credentials",
    type: "Type",
    username: "Username",
    description: "Description",
    addCredential: "Add Credential",
    searchPlaceholder: "Search by name or remark",
    nodeSearchPlaceholder: "Search IP, Host or Domain",
    yes: "Yes",
    no: "No",
    save: "Save",
    cancel: "Cancel",
    required: "Required",
  },
};
